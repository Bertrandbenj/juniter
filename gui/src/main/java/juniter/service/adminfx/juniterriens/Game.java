...
import juniter.service.adminfx.juniterriens.engine.Gate;
import juniter.service.adminfx.juniterriens.engine.Obstacle;
...
public class Game implements Initializable {
    ...
    private Timeline entry() {
        ...
        // Init map objects
        trm = new Curiosity("/adminfx/game/trm.jpeg", "2,718, but why?", 300, 20);
        trmPage = new Curiosity("/adminfx/game/blankpage.png", "Hmmm, a TRM page covered in blood !", 600, 20);
        duniter = new Curiosity("/adminfx/game/duniter.png", "A protocol, a software, and more ...", 100, 400);
        puits = new Curiosity("/adminfx/game/mariopipe.png", List.of("- C'est Marcel!", "- Kimamila?"), (int) canvas.getWidth() / 2 + 50, (int) canvas.getHeight() / 2);
        obstacles.addAll(List.of(trm, trmPage, duniter, puits));

        // Init Gates objects
        rt = new Gate("/adminfx/game/gate.png", roundTable(gc), 50, 180);
        technology = new Gate("/adminfx/game/gate.png", technos(gc), 1000, 180);
        ...
        exercise1.getKeyFrames().add(new KeyFrame(Duration.millis(16), event -> {
            // game logic
    ...
    private Timeline technos(GraphicsContext gc) {
        ...
        var main = new Gate("/adminfx/game/gate.png", null, canvas.getWidth() - 50, canvas.getHeight() / 2);
    ...
    private Timeline roundTable(GraphicsContext gc) {
        ...
        var moul = new Curiosity("/adminfx/game/blueWizard.png", list, 600, 50);
        ...
        anim.getKeyFrames().add(new KeyFrame(Duration.millis(16), event -> {
            ...
            // creats seats
            for (int i = 0; i < 59; i++) {
                var img = new Image("/adminfx/game/moneybag2.png");
