...
public class AvroService {
    ...
    private Schema SCHEMAB = ArchiveBINDEX.getClassSchema(),
            SCHEMAC = ArchiveCINDEX.getClassSchema(),
            SCHEMAI = ArchiveIINDEX.getClassSchema(),
            SCHEMAM = ArchiveMINDEX.getClassSchema(),
            SCHEMAS = ArchiveSINDEX.getClassSchema(),
            SCHEMABLOCKS = ArchiveBlock.getClassSchema(),
            SCHEMABLOCKS2 = DBBlock2.getClassSchema();
...
//        Schema schema = ReflectData.get().getSchema(DBBlock.class);
//
//        LOG.info(schema.toString(true));

        try (Stream<DBBlock> streamM = blockService.streamBlocksFromTo(1, 100)) {

            var bout = new DataFileWriter<>(new GenericDatumWriter<DBBlock2>())
                    .setCodec(CodecFactory.deflateCodec(9))

                    .create(SCHEMABLOCKS2, new File(AVRO_DATA, "chainDump.avro"));

            streamM.map(s -> {
                var res = modelMapper.map(s, DBBlock2.class);

                res.setInnerHash(s.getInner_hash());
                res.setDividend(s.getDividend());
